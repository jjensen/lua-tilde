...
namespace WeifenLuo.WinFormsUI.Docking
{
	...
	public delegate IDockContent DeserializeDockContent(string persistType, string persistString);
	...
	public partial class DockPanel : Panel
	{
		...
		/// <summary>
		/// Forwards a ProcessCmdKey message from a FloatWindow to the main application window,
		/// so menu shortcuts work.
		/// </summary>
		/// <remarks>
		/// The shortcut seems to be recognised only when the originating window handle is from a child
		/// of the window with the MainMenu.
		/// </remarks>
		internal bool ForwardCmdKey(ref Message msg, Keys keyData)
		{
			Message fakemsg = Message.Create(this.Handle, msg.Msg, msg.WParam, msg.LParam);
			return ProcessCmdKey(ref fakemsg, keyData);
		}
	}
}
