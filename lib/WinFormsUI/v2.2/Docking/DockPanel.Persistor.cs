...
namespace WeifenLuo.WinFormsUI.Docking
{
    partial class DockPanel
    {
        private static class Persistor
        {
            ...
            {
                private string m_persistType;
                public string PersistType
                {
                    get { return m_persistType; }
                    set { m_persistType = value; }
                }
            ...
            public static void SaveAsXml(DockPanel dockPanel, Stream stream, Encoding encoding, bool upstream)
            {
                ...
                foreach (IDockContent content in dockPanel.Contents)
                {
                    ...
                    xmlOut.WriteAttributeString("PersistType", content.DockHandler.Form.GetType().AssemblyQualifiedName);
                    ...
                    if (content.DockHandler.PersistString != null)
                        xmlOut.WriteRaw(content.DockHandler.PersistString);
            ...
            private static ContentStruct[] LoadContents(XmlTextReader xmlIn)
            {
                ...
                for (int i = 0; i < countOfContents; i++)
                {
                    ...
                    contents[i].PersistType = xmlIn.GetAttribute("PersistType");
                    ...
                    contents[i].PersistString = xmlIn.ReadInnerXml();
                }
            ...
            public static void LoadFromXml(DockPanel dockPanel, Stream stream, DeserializeDockContent deserializeContent, bool closeStream)
            {
                ...
                {
                    IDockContent content = contents[i].PersistType == null ? null : deserializeContent(contents[i].PersistType, contents[i].PersistString);
                    if (content == null)
